<!-- components/banner/ListPromo.vue -->
<template>
    <div
            ...
            <div class="lg:flex lg:items-center lg:justify-between">
                <div class="flex flex-col opacity-80">
                    <span class="text-[20px] font-bold leading-[100%] text-[#000000] lg:text-[24px]">
                        <strong>Nhiều chương trình ưu đãi</strong>
                    </span>
                    <div class="mb-5">
                        <span
                            class="font-normal font-monasans text-[14px] leading-[100%] text-[#6B6B6B] lg:text-[16px]">
                            Deal săn thả ga - Shopping không lo giá
                        </span>
                    </div>
                </div>
            </div>
            <div class="relative w-full overflow-hidden rounded-3xl bg-white " @mouseenter="pause" @mouseleave="play">
                <!-- Track -->
                <div class="flex h-[380px] select-none md:h-[460px]" :style="trackStyle" @pointerdown="onPointerDown"
                    ...
                    <!-- Slide -->
                    <component :is="name" v-for="(name, i) in slides" :key="name" />
                </div>
                ...
                <div class="absolute bottom-4 left-1/2 flex -translate-x-1/2 items-center gap-2">
                    <button v-for="(s, i) in slides" :key="'dot-' + i"
                        ...
                        :class="i === index ? 'w-8 bg-slate-900/90' : 'w-2'" :aria-label="`Go to slide ${i + 1}`"
                        @click="goTo(i)" />
                </div>
            </div>
        </div>
...
const slides = ['BannerItemSliderDesktop5', 'BannerItemSliderDesktop7']
